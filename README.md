...
也仿造了Spring的一些功能，例如基于注解的控制器Controller，也包含了过滤器，拦截器等功能，用户可根据具体的业务扩展接口。
...
3. 因为注解是要发现的，所有需要包扫描。默认的包扫描路径就是启动类所在的包路径，当然，这些也是可配置的。（配置项附在最后）
...
controllerPackage | 扫描控制器所在的包路径 | 启动类的根路径,例如 top.yeonon | 重要
filterPackage | 扫描过滤器所在的包路径 |启动类的根路径,例如 top.yeono | 重要
interceptorPackage | 扫描拦截器所在的包路径 | 启动类的根路径,例如 top.yeono | 重要
